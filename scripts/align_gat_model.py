#!/usr/bin/env python3
"""Model-level alignment: GAT (full model, Torch -> Keras weight transfer).

Builds both Torch GATModel and a KerasGATFullStack that mirrors the full
architecture (Embedding + Dense + multi-head AttentionHeadGAT + PoolingNodes +
...
class KerasGATFullStack:
    """Full Keras GAT model stack mirroring GATModel architecture."""
    ...
    def __init__(self, cfg: Config):
        ...
        for _ in range(cfg.depth):
            ...
            for _ in range(cfg.heads):
                layer_heads.append(KerasAttentionHeadGAT(
...
def transfer_all_weights(torch_model: GATModel, keras_stack: KerasGATFullStack, cfg: Config):
...
def main():
    ...
    # Compare
    print(f"GAT model-level alignment (Torch -> Keras):")
    compare_outputs("GAT_output", torch_out, keras_out, MAX_MAE, MAX_ABS)
    print("GAT model alignment PASSED.")
