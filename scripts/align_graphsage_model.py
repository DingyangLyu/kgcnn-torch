#!/usr/bin/env python3
"""Model-level alignment: GraphSAGE (full model, Torch -> Keras weight transfer).

Builds both Torch GraphSAGEModel and a KerasGraphSAGEFullStack that mirrors the
full architecture (Embedding + per-layer (GatherOutgoing + Concat + edge_mlp +
AggregateLocal + Concat + node_mlp + LayerNorm) + PoolingNodes + output MLP),
...
from model_alignment_utils import (
    copy_dense, copy_embedding, copy_mlp, copy_layernorm,
...
class KerasGraphSAGEFullStack:
    """Full Keras GraphSAGE model stack mirroring GraphSAGEModel architecture."""
    ...
    def __init__(self, cfg: Config):
        ...
        edge_act = ["relu"] * max(len(cfg.edge_mlp_units) - 1, 0) + ["linear"]
        node_act = ["relu"] * max(len(cfg.node_mlp_units) - 1, 0) + ["linear"]
    ...
    def forward(self, z, edge_attr, edge_index, batch_id_node, batch_id_edge,
        ...
        for i in range(self.cfg.depth):
            eu = self.gather([n, edge_index])
            eu = self.concat([eu, edge_attr])
            eu = self.edge_mlps[i]([eu, batch_id_edge, count_edges])
            nu = self.aggrs[i]([n, eu, edge_index])
            hu = self.concat([n, nu])
            n = self.node_mlps[i]([hu, batch_id_node, count_nodes])
            n = self.norms[i]([n, batch_id_node, count_nodes])
...
def transfer_all_weights(torch_model: GraphSAGEModel,
                         keras_stack: KerasGraphSAGEFullStack, cfg: Config):
    copy_embedding(torch_model.node_embedding, keras_stack.node_embedding)

    for i in range(cfg.depth):
        copy_mlp(torch_model.edge_mlps[i], keras_stack.edge_mlps[i])
        copy_mlp(torch_model.node_mlps[i], keras_stack.node_mlps[i])
        copy_layernorm(torch_model.layer_norms[i].ln, keras_stack.norms[i])
...
def main():
    ...
    print(f"GraphSAGE model-level alignment (Torch -> Keras):")
    compare_outputs("GraphSAGE_output", torch_out, keras_out, MAX_MAE, MAX_ABS)
    print("GraphSAGE model alignment PASSED.")
