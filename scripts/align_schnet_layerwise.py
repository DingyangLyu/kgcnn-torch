#!/usr/bin/env python3
"""Layerwise numeric alignment check: Keras SchNetInteraction vs torch SchNet."""
import os
import sys
...
def torch_forward_stages(model: SchNetModel, x, edge_attr, edge_index) -> Dict[str, torch.Tensor]:
    out = {}
    h = model.dense_in(x)
    out["dense_in"] = h.detach().cpu()
    for i, layer in enumerate(model.interactions):
        h = layer(h, edge_attr, edge_index)
...
def main():
    ...
    keras_stack = KerasSchNetStack(cfg)
    ...
    for i in range(cfg.depth):
        ...
        copy_dense_torch_to_keras(t.dense_in, k.lay_dense1)
        copy_dense_torch_to_keras(t.cfconv.dense1, k.lay_cfconv.lay_dense1)
        copy_dense_torch_to_keras(t.cfconv.dense2, k.lay_cfconv.lay_dense2)
        copy_dense_torch_to_keras(t.dense1, k.lay_dense2)
        copy_dense_torch_to_keras(t.dense2, k.lay_dense3)
