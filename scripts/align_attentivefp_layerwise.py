#!/usr/bin/env python3
"""Layerwise numeric alignment check: Keras AttentiveFP atom stack vs torch."""
import os
import sys
...
def copy_gru_torch_to_keras(torch_gru_cell: torch.nn.GRUCell, keras_gru_update: KerasGRUUpdate):
    ...
    kernel = _reorder_chunks(w_ih, axis=0).T           # (in, 3u)
    recurrent_kernel = _reorder_chunks(w_hh, axis=0).T # (u, 3u)
    bias_input = _reorder_chunks(b_ih, axis=0)         # (3u,)
    bias_recurrent = _reorder_chunks(b_hh, axis=0)     # (3u,)
...
def torch_forward_stages(model: AttentiveFPModel, x, edge_attr, edge_index) -> Dict[str, torch.Tensor]:
    out = {}
    h = model.dense_in(x)
    out["dense_in"] = h.detach().cpu()
    for i in range(model.depth_ato):
        c = model.attention_layers[i](h, edge_index, edge_attr)
        h = model.gru_updates[i](c, h)
...
def main():
    ...
    keras_stack = KerasAttentiveFPAtomStack(cfg)
    ...
    for i in range(cfg.depth_ato):
        ...
        copy_gru_torch_to_keras(torch_model.gru_updates[i].gru_cell, keras_stack.grus[i])
